...
export default function FormBugBounty() {
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of files) {
        if (newTotalFileSize + file.size > 100 * 1024 * 1024) {
          setErrors((prev) => ({
            ...prev,
            files: "حجم کل فایل‌ها نباید بیشتر از 100 مگابایت باشد.",
          }));
          return;
        }
      ...
      setUploadedFiles((prevFiles) => {
        ...
        const imageFiles = combinedFiles.filter((file) =>
          ["image/jpeg", "image/png"].includes(file.type)
        );
        const videoOrZipFiles = combinedFiles.filter(
          (file) =>
            [
              "video/mp4",
              "application/zip",
        ...
        if (videoOrZipFiles.length > 1) {
          setErrors((prev) => ({
            ...prev,
            files: "فقط یک ویدیو یا فایل ZIP مجاز است.",
          }));
          return prevFiles; // بازگرداندن فایل‌های قبلی در صورت بروز خطا
        }
  ...
  ); // اضافه کردن ‘uploadedFiles’ به دیپندنسی‌ها
  ...
  const validateForm = useCallback(async () => {
    try {
      await validationSchema.validate(formDataRef.current, {
        abortEarly: false,
      });
      setErrors({});
      return true;
  ...
  return (
    <div className=" rounded-[30px] w-full py-10 px-5 bg-background">
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 ">
        ...
        <div>
          ...
          <div className="relative z-10 h-[160px] lg:h-[227px] border border-dashed lg:border-solid border-gray-300 rounded-xl mb-4">
            <div className="absolute  inset-0 flex flex-col items-center justify-center pt-[14px] pb-[11px] lg:pt-[37px] lg:pb-[21px]">
              ...
              <span className="text-[9px] md:text-sm font-normal opacity-50 text-center text-sixth">
                حداکثر ۵ تصویر jpeg یا PNG ،یک ویدیو MP4 یا یک فایل Zip ،حداکثر
                حجم کل فایل ها 100 مگابایت باشد
              </span>
            </div>
            <input
              type="file"
        ...
        <div className="flex items-start mb-[19px]">
          ...
          <label
            ...
            className={`w-5 h-5 lg:w-9 lg:h-9 flex items-start justify-center rounded lg:rounded-lg border border-[#ADADAD80] cursor-pointer ${
        ...
        <button
          type="submit"
          className="w-[124px] h-[40px] lg:w-[232px] lg:h-[75px] bg-primary text-white px-4 py-2 rounded-xl self-end text-[13.44px] lg:text-[25px]"
        >
          ارسال درخواست
        </button>
      </form>
