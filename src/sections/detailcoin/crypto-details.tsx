    ...
    <Link href={`/coins/${symbol}`}>
      <div className="flex justify-between items-center">
        ...
        <div className="flex flex-col text-end gap-1">
          ...
          {isLoading ? <Skeleton baseColor={baseColor} highlightColor={highlightColor} width={50} height={18} /> : <p dir="ltr" className={`text-lg font-semibold text-left ${priceChangePercent.includes('-') ? 'text-[#F00500]' : 'text-[#33B028]'}`}>{priceChangePercent} %</p>}
        </div>
      </div>
    </Link>
  );
