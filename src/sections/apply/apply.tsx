...
import FormField from "../job/input/InputField";
...
export default function ApplyPage({ title }: { title: string }) {
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
        ...
        if (file.size > 200 * 1024 * 1024) {
          setErrors((prev) => ({
            ...prev,
            file: "حجم فایل نباید بیشتر از 200 مگابایت باشد.",
          }));
          return;
        }
        if (file.type !== "application/pdf") {
          setErrors((prev) => ({
            ...prev,
            file: "فقط فایل‌های PDF مجاز هستند.",
          }));
          return;
        }
  ...
  return (
    <div className="base-style w-full pt-20">
      <div className="w-1/2">
        <h1 className="text-xl font-bold text-start mb-3">
          فرم ارسال درخواست و رزومه
        </h1>
        <h2 className="text-xs font-semibold opacity-50 mb-8">{title}</h2>
        <form
          onSubmit={handleSubmit}
          className="flex flex-col items-center justify-center sm:w-1/2 sm:min-w-[470px]"
        >
          <div className="w-full ">
            ...
            <div>
              ...
              {!uploadedFileName ? (
                <p className="text-sm text-gray-500 mt-2">
                  مشخصات فایل: pdf، حجم فایل کمتر از 20 مگابایت
                </p>
              ) : (
                <div className="mt-2">
            ...
            <button
              type="submit"
              className=" w-full h-10 sm:h-16 bg-primary text-white px-4 py-2 rounded-xl  mt-12"
            >
              ارسال درخواست
            </button>
          </div>
