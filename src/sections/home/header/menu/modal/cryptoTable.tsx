    ...
    <div className="w-full z-50">
      ...
      >
        {filterButtons.map((btn) => (
          <button
            key={btn.key}
            className={`ml-2 px-2 py-1 text-xs font-semibold rounded-lg whitespace-nowrap text-center ${
              filter === btn.key
          ...
          {filteredData.length > 0 ? (
            filteredData.map((crypto, index) => (
              <Link href={`/coins/${crypto.symbol}`} key={index}>
                <div className="ml-2 flex justify-between items-center border-b border-[#ADADAD80] py-2  text-sm">
                  <div className="w-1/3 flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs font-semibold text-sixth opacity-50 ">
                        {crypto.symbol}
                      </span>
                    </div>
                  </div>
                  <div
                    dir="ltr "
                    className={`flex  w-1/3 text-xs font-semibold ${crypto.changeColor} pr-4`}
                  >
