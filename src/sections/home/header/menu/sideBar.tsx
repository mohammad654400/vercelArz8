...
export default function SideBar({ close }: { close: () => void }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-60 bg-background rounded-tl-3xl rounded-bl-3xl shadow-xl transform transition-transform duration-400 ease-in-out ${isOpen ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"}`}
        style={{ willChange: "transform, opacity" }} // Helps browser optimize animations
      >
        <div className="fixed flex top-0 right-0 z-50 h-full xl:hidden w-60 text-xs">
          <div className="right-0 px-6 py-2 sm:py-3 top-0 w-full h-full bg-background rounded-tl-3xl rounded-bl-3xl">
            ...
            <div className="mt-4 z-50 flex flex-col gap-4">
              ...
              <Link href="/job"><p>فرصت های شغلی</p></Link>
              <Link className="pt-2" onClick={handleClose} href="https://arz8.com/blog"><p>بلاگ</p></Link>
              <Link href="https://app.arz8.com/auth/login" className="mt-5 sm:mt-10 w-full bg-primary text-sm sm:text-xl font-semibold py-1 leading-7 sm:leading-10 text-white rounded-[9.85px] sm:rounded-2xl text-center transition-all">
                ورود / عضویت
              </Link>
            </div>
          </div>
