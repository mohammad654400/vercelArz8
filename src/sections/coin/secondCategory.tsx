...
export default function SecondCategory({ open, setOpen, data, infoMap, }: CategoryProps) {
  ...
  return (
    <div className="py-[30px]">
      <div
        className={`
    ${!open
            ? "w-full pt-4 md:pt-6 rounded-2xl mx-2 px-3 md:px-6"
    ...
    h-[283px] bg-[#F6F6F6] dark:bg-[#242428] pb-4`}
      ...
      >
        <div
          className={`relative ${!open ? "flex justify-between cursor-pointer" : "flex justify-center pt-4"}`}>
          <h1 className={`mb-2 ${!open ? "block" : "hidden"}`}>جدیدترین ارزهای ما</h1>
        ...
        {displayedCurrencies?.length === 0 ?
          ...
          : displayedCurrencies.map((item: CategoryItem, index: number) => (
            <div key={index} className={` ${!open ? "border-b border-[#ADADAD80]" : "border-none"} ${(index + 1) % 3 == 0 ? "border-none" : "border-b border-[#ADADAD80]"}`}>
              <div>
                <Link href={`price-cryptocurrencies/${item.symbol}`} className="flex justify-between items-center gap-x-3 md:gap-x-5 my-[18px]">
